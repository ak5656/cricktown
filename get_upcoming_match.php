...
function readOldResponse() {
    $fileData = file_get_contents("./getUpcomingMatchesData.txt") or die("Unable to open file!");
    return $fileData;
}


function isLatestData($currentTime, $lastCallTime) {
    $maxOldTime = 900; // second;
...
{   $BASEPATH =  __DIR__;
    $myfile = fopen($BASEPATH . "/" .  "getUpcomingMatchesData.txt", "w") or die("Unable to open file!");
